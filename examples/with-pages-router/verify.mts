...
assert.match(environmentExample, /^APP_NAME="Pages Router Example"$/m);
...
assert.match(route, /response\.status\(500\)\.json\(\{ ok: false \}\)/);
assert.equal(
  /response\.status\(500\)\.json\([^)]*diagnostics/s.test(route),
  false
);
