...
import { createSharedReadiness } from './src/app/environment-bootstrap';
import {
...
const blankAnalytics = checkBrowser(
  deploymentSource({ ...profiles[0], ANALYTICS_ID: '' })
);
assert.equal(blankAnalytics.ok, false);
...
assert.equal(defaultedServer.value.maxConnections, 10);
assert.equal(defaultedServer.value.secretKey, undefined);
assert.deepEqual(parseConfiguration('{invalid-json'), {});
...
const invalidCanary = 'never-disclose-invalid-database';
...
const asynchronousSchema: StandardSchemaV1<string, string> = {
  '~standard': {
    ...
    vendor: 'asynchronous-negative-fixture',
    version: 1
  }
};
const asynchronousResult = await checkServer(deploymentSource(profiles[0]), {
  databaseUrl: asynchronousSchema
});
assert.equal(asynchronousResult.ok, false);
if (!asynchronousResult.ok) {
  assert.match(
    JSON.stringify(asynchronousResult.diagnostics),
...
assert.equal(
  /next-dynamic-env|COPY packages|packages\/next-dynamic-env/.test(dockerfile),
  false
);
...
const route = await readFile('src/app/api/env/route.ts', 'utf8');
...
assert.equal(
  /setInterval|waitForEnv|__NEXT_DYNAMIC_ENV__/.test(instrumentation),
  false
);
